...
system::hal::drivers::ports::port8_t idt::PIC_master;
system::hal::drivers::ports::port8_t idt::PIC_master_ctrl;

system::hal::drivers::ports::port8_t idt::PIC_second;
system::hal::drivers::ports::port8_t idt::PIC_second_ctrl;
...
{
	PIC_master.init(0x20, system::hal::drivers::ports::type::output);
	PIC_master_ctrl.init(0x21, system::hal::drivers::ports::type::output);

	PIC_second.init(0xA0, system::hal::drivers::ports::type::output);
	PIC_second_ctrl.init(0xA1, system::hal::drivers::ports::type::output);
	...
	PIC_master = 0x11;
    PIC_second = 0x11;
    PIC_master_ctrl = 0x20;
    PIC_second_ctrl = 0x28;
    PIC_master_ctrl = 0x04;
    PIC_second_ctrl = 0x02;
    PIC_master_ctrl = 0x01;
    PIC_second_ctrl = 0x01;
    PIC_master_ctrl = 0x0;
    PIC_second_ctrl = 0x0;
...
__cdecl void fault_handler(idt::regs32_t regs)
{
	...
	fatal(exceptions[regs.identifier.trap_number]);
}
...
{
	dprint_info("enable_interrupts() >> interrupts enabled");
	asm volatile("sti");
}
void idt::manager_t::disable_interrupts()
{
	dprint_info("disable_interrupts() >> interrupts disabled");
	asm volatile("cli");
}
